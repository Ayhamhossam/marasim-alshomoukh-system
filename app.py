...
def purchases():
    ...
    if request.method == 'POST':
        ...
        existing = c.execute("SELECT id, quantity FROM products WHERE name=?", (name,)).fetchone()

        if existing:
            new_qty = existing[1] + qty
            c.execute("UPDATE products SET quantity=?, purchase_price=? WHERE id=?",
...
def sales():
    ...
    if request.method == 'POST':
        ...
        current_qty = c.execute("SELECT quantity FROM products WHERE id=?",
                                (product_id,)).fetchone()[0]

        if qty > current_qty:
            return "الكمية غير متوفرة في المخزون"
