...
def dashboard():
    ...
    total_sales = c.execute("SELECT SUM(quantity_sold*sale_price) FROM sales").fetchone()[0] or 0
    conn.close()
    return render_template("dashboard.html", total_products=total_products,
                           total_stock=total_stock, total_sales=total_sales)
...
def purchases():
    ...
    if request.method=='POST':
        ...
        product = c.execute("SELECT id, quantity FROM products WHERE name=?",(name,)).fetchone()
        if product:
            new_qty = product[1]+qty
            c.execute("UPDATE products SET quantity=?, purchase_price=? WHERE id=?",(new_qty, price, product[0]))
        else:
            c.execute("INSERT INTO products (name,type,purchase_price,quantity) VALUES (?,?,?,?)",
...
def sales():
    ...
    if request.method=='POST':
        ...
        c.execute("INSERT INTO sales (product_id,quantity_sold,sale_price,date) VALUES (?,?,?,?,?)",
...
def returns_page():
    ...
    if request.method=='POST':
        ...
        if reason=="مرتجع زبون":
            ...
            c.execute("UPDATE products SET quantity = quantity - ? WHERE id=?",(qty,product_id))
        c.execute("INSERT INTO returns (product_id,quantity,reason,date) VALUES (?,?,?,?)",
